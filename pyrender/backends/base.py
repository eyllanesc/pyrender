...
class BaseViewer:
    ...
    def __init__(self,
        ...
        self._axis_mesh = Mesh.from_trimesh(
            trimesh.creation.axis(origin_size=0.1,
                                  axis_radius=0.05,
                                  axis_length=1.0),
            smooth=False,
        )
    ...
    def save_gif(self, filename=None):
        ...
        """
        if filename is None:
            filename = self.get_save_filename(["gif", "all"])
    ...
    def handle_resize(self, width, height):
        ...
        if self._renderer is None or width <= 0 or height <= 0:
            return
    ...
    def handle_mouse_press(self, buttons, modifiers, x, y):
        """Record an initial mouse press."""
        self._trackball.set_state(Trackball.STATE_ROTATE)
        if buttons & Buttons.LeftButton:
            ctrl = modifiers & Modifiers.ControlModifier
            shift = modifiers & Modifiers.ShiftModifier
            if ctrl and shift:
        ...
        elif buttons & Buttons.MiddleButton:
            self._trackball.set_state(Trackball.STATE_PAN)
        elif buttons & Buttons.RightButton:
            self._trackball.set_state(Trackball.STATE_ZOOM)
    ...
    def handle_mouse_drag(self, buttons, modifiers, x, y):
        """Record a mouse drag.
        """
        if buttons != Buttons.NoButton:
            self._trackball.drag(np.array([x, y]))

    def handle_mouse_release(self, buttons, modifiers, x, y):
    ...
    def handle_key_press(self, key, native_key):
        """Record a key press."""
        # First, check for registered key callbacks
        for k in (key, native_key):
            if key in self.registered_keys:
                tup = self.registered_keys[key]
                callback = None
                args = []
                kwargs = {}
        ...
        # C toggles backface culling
            ...
            self.render_flags[
                "cull_faces"] = not self.render_flags["cull_faces"]
        ...
        # F toggles face normals
            ...
            self.viewer_flags[
                "fullscreen"] = not self.viewer_flags["fullscreen"]

            self.set_full_screen(self.viewer_flags["fullscreen"])
        ...
        # S toggles shadows
        elif key == "H" and sys.platform != "darwin":
        ...
        # M toggles face normals
            ...
            self.render_flags[
                "face_normals"] = not self.render_flags["face_normals"]
        ...
        # N toggles vertex normals
            ...
            self.render_flags[
                "vertex_normals"] = not self.render_flags["vertex_normals"]
        ...
        # R starts recording frames
            ...
            if self.viewer_flags["record"]:
                self.save_gif()
                self.set_title(self.viewer_flags["window_title"])
            else:
                self.set_title("{} (RECORDING)".format(
    ...
    def handle_timeout(self):
        ...
        if self.viewer_flags["record"]:
            self._record()
        if self.viewer_flags[
                "rotate"] and not self.viewer_flags["mouse_pressed"]:
            self._rotate()
    ...
    def _save_image(self):
        filename = self.get_save_filename(["png", "jpg", "gif", "all"])
        if filename is not None:
            self.viewer_flags["save_directory"] = os.path.dirname(filename)
    ...
    def _rotate(self):
        ...
        az = self.viewer_flags["rotate_rate"] / self.viewer_flags[
            "refresh_rate"]
